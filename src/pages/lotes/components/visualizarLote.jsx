...
const VisualizarLote = () => {
    ...
    const [itemsPerTablePage] = useState(7);

    // Add this function before the return statement
    const indexOfLastItem = currentTablePage * itemsPerTablePage;
    const indexOfFirstItem = indexOfLastItem - itemsPerTablePage;
    const currentItems = seguimiento.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(seguimiento.length / itemsPerTablePage);
                        ...
                        {lote && (
                            <div className="grid grid-cols-2 gap-4 border border-gray-300 p-4 shadow-lg">
                                <div>
                                    ...
                                    <p><strong>Estado del lote:</strong> {lote.status == 'Rechazado' ? (<strong className="text-red-600">RECHAZADO</strong>) : lote.status == 'Cosechado' ? (<strong className="text-green-600">RECHAZADO</strong>) : lote.status}</p>
                                    <p>
                                                ...
                                                <div className="flex justify-between items-center px-4 py-3 bg-white border-t border-gray-200 sm:px-6">
                                                    <div className="flex items-center">
                                                        <p className="text-sm text-gray-700">
                                                            Mostrando {indexOfFirstItem + 1} a {Math.min(indexOfLastItem, seguimiento.length)} de {seguimiento.length} registros
                                                        </p>
                                                    </div>
                                                    <div className="flex items-center space-x-2">
                                                        <button
                                                            ...
                                                            className={`px-3 py-1 rounded-md ${currentTablePage === 1 ? 'bg-gray-100 text-gray-400' : 'bg-[#168C0DFF] text-white'}`}
                                                        >
                                                            Anterior
                                                        </button>
                                                        <span className="px-3 py-1 text-gray-700">
                                                            Página {currentTablePage} de {totalPages}
                                                        </span>
                                                        <button
                                                            onClick={handleNextTablePage}
                                                            disabled={currentTablePage === totalPages}
                                                            className={`px-3 py-1 rounded-md ${currentTablePage === totalPages ? 'bg-gray-100 text-gray-400' : 'bg-[#168C0DFF] text-white'}`}
                                                        >
                                                            Siguiente
                                                        </button>
                                                    </div>
